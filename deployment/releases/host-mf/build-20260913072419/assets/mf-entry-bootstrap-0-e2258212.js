...
(async () => {
  const __mfHostInit = await __mfImport("./hostInit-BMqYi4UZ.js");
  await __mfHostInit.__tla;
  const { initHost } = __mfHostInit;
  
  const runtime = await initHost();
  const __mfPreloadRemote = (runtimeRemote, remote) => {
    const remoteCacheKey = "virtual:mf:__mfe_internal__host_app__mf_owner__1__mf_v__runtimeInit__mf_v__.js::" + remote;
  ...
  const __mfRemotePreloads = [__mfPreloadRemote("__mfe_internal__host_app__mf_owner__1__cart_app", "cart_app"),__mfPreloadRemote("__mfe_internal__host_app__mf_owner__1__product_app", "product_app")];
...
})().then(() => __mfImport("./index-DYFxj3h0.js"));
